...
module.exports.reset = function(req,res){
    new formidable.IncomingForm().parse(req, async (err, fields) => {
        ...
        {
            const user_id = await User.findOne({where:{mobileNumber:mobile}});        
            User.update({
            ...
            .then(result => {
                if(result != 0){
                    ...
                    res.status(200).header('Authorization',token).json({status:"true",message:"User password updated sucessfully"});
                }
                else
                {
