                ...
                if(mobile != null)
                {
                    ...
                    .then(result =>{
                        if(result)
                        {
                            ...
                            return res.send(JSON.stringify({status:"true",message:"OTP send to you mobile number",mobile:mobile}));
                        }
                    })
                    .catch(err=>{
                    ...
                    return res.send(JSON.stringify({status:"false",message:"No mobile number found on you record please contact admin"}));
                }
            }
            else{
                return res.send(JSON.stringify({status:"false",message:"Enter valid email address"}));
            }
        })
        .catch(err=>{
            return res.send(JSON.stringify({error:err}));
